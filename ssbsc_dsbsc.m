...
dsb_sc = ((Am*Ac)/2)*cos(2*pi*(fm+fc)*t); %dsb sc modulated wave
...
subplot(4,1,3);
plot(t, dsb_sc);
title("MODULATED SSB SC SIGNAL (TIME DOMAIN)");
xlabel('time (sec)');
ylabel('amplitude');
grid on;
subplot(4,1,4);
plot(t, msg_r);
title("DEMODULATED SSB SC SIGNAL (TIME DOMAIN)");
xlabel('time (sec)');
ylabel('amplitude');
grid on;
figure;
...
plot(f, abs(DSB_SC));
xlim([-15 15]);
title('SSB SC MODULATION IN FREQUENCY DOMAIN');
xlabel('frequency(hz)');
ylabel('amplitude');
grid on;
subplot(2,1,2);
plot(fr, abs(MSG_R));
xlim([-6 6]);
title('SSB SC DEMODULATION IN FREQUENCY DOMAIN');
xlabel('frequency(hz)');
ylabel('amplitude');
grid on;
